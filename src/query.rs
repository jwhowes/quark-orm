...
use crate::{error::DbError, model::Model};

pub trait ExecQuery<M: Model, O> {
    fn exec(self, db: &PgPool) -> impl Future<Output = Result<O, DbError>>;
}

pub struct FindFirst<M: Model>(pub M::Filter);

impl<M: Model> ExecQuery<M, Option<M>> for FindFirst<M> {
    async fn exec(self, db: &PgPool) -> Result<Option<M>, DbError> {
...
impl<M: Model> ExecQuery<M, Vec<M>> for FindMany<M> {
    async fn exec(self, db: &PgPool) -> Result<Vec<M>, DbError> {
